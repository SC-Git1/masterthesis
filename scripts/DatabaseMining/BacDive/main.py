...
if __name__ == '__main__':
    ...
    dfEntry = pd.read_csv("bacdive_entry_df.csv", delimiter=",", header = 0)
    ...
    # remove duplicates i.e., points with the same source id, temperature value and temperature annotation type
    dfEntry.drop_duplicates(subset=["BacDiveID","type", "temperature"], inplace= True, ignore_index=True)
    # print(len(dfEntry)) # 13277 records remain after removing duplicates
    ...
    # get all BacDiveIDs included in optimal subset
    ...
    # print(len(dfNew)) # -> there are 9543 remaining records
    Taxids = dfNew["id"]
    ...
    with open("NCBI_Annotated_BacDive_Corr.tsv", "w") as f:
        # header
        f.write("Source_ID" + "\t" + "ncbiTaxID" + "\t" + "Name" + "\t" + "Type" + "\t" + "Temp" + "\t" + "Superkingdom"
        ...
        for i in range(len(Taxids)):
            ...
            try:
                ...
                pass
            # write to file
            f.write(str(BacDiveIDs[i]) + "\t" + el + "\t" + Species[i] + "\t" + Types[i] + "\t" + str(Temps[i]) + "\t" +
